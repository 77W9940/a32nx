...
class ThrustLimits_A380X {
 ...
 public:
  ...
  // [level][tatRow][altCol], level 0 = DCL1, 1 = DCL2, 2 = DCL3
  static constexpr double dclThr[3][19][12] = {{
      // DCL1
  ...
  static double climbDerateFactor(int level, double tat, double altitude) {
    if (level < 1 || level > 3) return 100.0;
    return bilinearLookup(dclTatBreakpoints, dclAltBreakpoints, &dclThr[level - 1][0][0], 19, 12, tat, altitude);
  }
