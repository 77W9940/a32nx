 ...
 * @param flightPlanInterface used to propagate cruise winds along the route if necessary.
...
export function formatWindRequest(
  ...
  if (shouldRequestCruiseWinds && cruiseLegs.length > 0) {
    const propagatedWinds = flightPlanInterface.propagateWindsAt(0, [], planIndex);
    ...
    if (flightLevels.length === 0) {
      ...
      // Still nothing to request winds for (e.g. no cruise step and route propagation found nothing) -
      // request a spread of levels around the planned cruise level so the crew gets *something* useful back.
      if (cruiseLevel !== null) {
        const fillerLevels = [cruiseLevel - 20, cruiseLevel + 20, cruiseLevel + 40, cruiseLevel + 60];
        for (const level of fillerLevels) {
          if (flightLevels.length >= 4) {
            break;
          }
          if (level > 0 && !flightLevels.includes(level)) {
            flightLevels.push(level);
          }
        }
      }
    }
